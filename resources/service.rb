...
property :lsb_init_dir, String, default: lazy { node['runit']['sb_init_dir'] || '/etc/init.d' }
...
property :status_command, String, default: lazy { |r| "#{r.sv_bin} status #{r.service_dir}" }
property :sv_templates, [true, false], default: true
property :sv_timeout, Integer
property :sv_verbose, [true, false], default: false
property :log_dir, default: lazy { |r| ::File.join('/var/log/', r.service_name) }
...
action :create do
  ...
  unless run_context.nil?
    ...
    @service_mirror.start_command("#{new_resource.sv_bin} start #{service_dir_name}")
    @service_mirror.stop_command("#{new_resource.sv_bin} stop #{service_dir_name}")
    @service_mirror.restart_command("#{new_resource.sv_bin} restart #{service_dir_name}")
    @service_mirror.status_command("#{new_resource.sv_bin} status #{service_dir_name}")
